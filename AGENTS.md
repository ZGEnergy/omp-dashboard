...
| `src/client/components/ServerSelector.tsx` | Server selector dropdown showing persisted known servers with eager availability probing (mount + on-open + on-knownServers-change + on-server-switch; no continuous timer). Accepts `inFlightSwitchKey` to render a spinner on the entry being switched to. Unreachable entries render dimmed with an "Unreachable" badge but remain clickable — the transactional switch handles failures safely. See change: safe-server-switch |
