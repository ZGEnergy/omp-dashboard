# honcho-memory-plugin Specification

## Purpose

This capability covers the **Honcho memory dashboard plugin** — a `pi-dashboard-plugin` shipped as `pi-memory-honcho-dashboard` that surfaces the `pi-memory-honcho` extension's TUI commands (setup, status, doctor, sync, mode, interview, map) as a dashboard settings panel and per-session-card affordances. The plugin gates all UI behind extension-installed detection, persists configuration to `~/.honcho/config.json` via atomic deep-merge writes, and exposes an auth-gated REST surface under `/api/plugins/honcho/`.
## Requirements
