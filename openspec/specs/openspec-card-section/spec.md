# openspec-card-section Specification

## Purpose

Session slot rendered inside proposal cards on the OpenSpec board. Each attached session row shows a status-tinted indicator, name, age, OpenSpec phase badge, and a token/context/cost stat line; exposes per-session lifecycle actions and an OpenSpec command menu; and visualizes worktree state with a task-progress delta against the proposal's main checkout.

> Note: this capability was previously removed by the `openspec-folder-card-ui` change and is reintroduced by the `redesign-openspec-board` change for the board surface.
