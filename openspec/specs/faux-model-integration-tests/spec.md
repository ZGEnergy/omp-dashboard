# faux-model-integration-tests Specification

## Purpose

Integration test coverage that drives the dashboard with a faux pi provider, so server-side streaming, client-side rendering, ask_user round-trips, and clean-VM smoke can be asserted without a real model API key.
