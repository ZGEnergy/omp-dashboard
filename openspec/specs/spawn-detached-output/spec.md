# spawn-detached-output Specification

## Purpose

Defines the stdio-capture contract for the `spawnDetached` primitive in `packages/shared/src/platform/detached-spawn.ts`. The primitive is used to spawn long-lived detached child processes (notably the dashboard server) where the parent must not retain pipes but the child's startup output must still be captured to a log file for diagnostics. This capability pins the routing of `opts.logFd` across the child's stdout AND stderr so that a single supplied descriptor produces a complete log regardless of which stream the child writes to.
