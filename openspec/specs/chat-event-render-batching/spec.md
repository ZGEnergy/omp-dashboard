# chat-event-render-batching

## Purpose

Coalesce bursts of live forwarded session events into a single per-frame state application so a burst of N events produces at most one React render pass instead of N, while preserving event order, final state, per-event side effects, and the existing replay path.
