# process-list-classification Specification

## Purpose
Server enriches forwarded `process_list` entries with `kind`/`label`/`sessionRef` via pid→session registry cross-reference and command-pattern matching, before forwarding as `process_list_update` to subscribed browsers.
