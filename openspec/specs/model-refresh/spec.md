...
Keep browser-side model lists in sync with provider credential changes on the server. When a provider is authenticated, an API key is saved, or a credential is removed, connected browsers must refresh their model lists so the model selector reflects the new set of available models without requiring a page reload.
