# Flow Step Type Tracking

## Purpose

Propagate flow step type information (`agent`, `fork`, `agent-decision`, `agent-loop-decision`, `conditional`, `flow-ref`) from the flow configuration through the event pipeline to dashboard UI components, so graph nodes and agent cards can render step-type-aware visuals (badges, prefixes, edge inference).
