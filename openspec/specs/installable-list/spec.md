...
Defines a single canonical package list at `~/.pi/dashboard/installable.json` shared across all dashboard starters, plus the merge-on-upgrade and server-side bootstrap reconciliation rules that drive idempotent install of required and optional packages.
