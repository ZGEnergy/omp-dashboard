...
- [ ] 1.1 In `packages/extension/src/session-sync.ts`, capture
...
- [ ] 1.2 Replace the two live reads `dashboardSpawned = !!process.env.PI_DASHBOARD_SPAWN_TOKEN`
  (session-sync.ts) and the one in `bridge.ts` with the captured boolean.
- [ ] 1.3 After reading the first-register token (`isFirstRegister` branch),
  `delete process.env.PI_DASHBOARD_SPAWN_TOKEN` so descendants don't inherit it.
- [ ] 1.4 Confirm the existing "first register only" + reattach/in-process-change
  behavior is unchanged (token still omitted on every non-first register).
- [ ] 1.5 Tests: first register emits token + `dashboardSpawned:true` and scrubs
...
- [ ] 2.1 In `packages/server/src/rpc-keeper/keeper.cjs` `spawnPi()`, after the
...
- [ ] 2.2 Verify `PI_DASHBOARD_SPAWNED=1` is still set on every (re)spawn so
  source labelling survives respawn via the capture-once boolean.
- [ ] 2.3 Keep `PI_KEEPER_PI_ARGS` / `PI_KEEPER_PI_CMD` stripping unchanged.
- [ ] 2.4 Tests (keeper suite): first launch env contains the token; simulated
  respawn env omits it; `PI_DASHBOARD_SPAWNED` present in both.

## 3. Cross-entry-point validation

- [ ] 3.1 Confirm REST spawn path (`packages/server/src/session-api.ts`) records
  `pendingClientCorrelations` / `pendingForkRegistry` exactly as the WS handler;
  no second injection of a reusable token.
- [ ] 3.2 Confirm `event-wiring.ts` `consume(spawnToken)` now resolves
  `spawnRequestId` on the first worktree register (no stale-token degrade).

## 4. Spec + docs

- [ ] 4.1 Update `spawn-correlation` spec delta (MODIFIED bridge requirement +
  ADDED scrub requirements) — see `specs/spawn-correlation/spec.md`.
- [ ] 4.2 Doc-only note on `spawnToken` field in `packages/shared/src/protocol.ts`:
...
- [ ] 5.1 Spawn an OpenSpec worktree session from the dashboard; placeholder card
  clears the moment the real card registers (not after 30 s).
...
- [ ] 5.4 `npm test` green — especially `dashboard-source-decision`,
