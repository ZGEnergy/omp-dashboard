...
- [ ] 1.1 In `App.tsx`, add `revealRequest: { sessionId: string; nonce: number }
...
- [ ] 1.2 Thread `seekToCard` into `SessionHeader` (App renders it above the
...
- [ ] 2.1 Add a "Seek to card" button to `SessionHeader` next to the session
...
- [ ] 3.1 Add `resolveFoldAncestors(session)` returning `{ workspaceId?, cwd,
...
- [ ] 3.2 Add `revealCard(session)` that GUARD-expands ancestors — only when the
      ...
      re-collapse).
- [ ] 3.3 Implement the reveal wait (copy scroll glue from the first-mount effect
...
- [ ] 3.4 Wire a `useEffect` keyed on `revealRequest?.nonce` that looks up the
...
- [ ] 4.1 Before reveal, classify the target up front from the `sessions` prop +
...
- [ ] 5.1 Extend the shared `Toast` (`packages/client/src/components/Toast.tsx`
...
- [ ] 5.2 Thread a retry dispatcher into `SessionList` (the reveal effect +
...
- [ ] T.1 Buried-reveal test (test-plan #F1): session under collapsed workspace +
      ...
      green after.
- [ ] T.2 Nonce re-fire test (test-plan #F2, #F4): dispatch two reveals same id
      ...
      drives re-fire.
- [ ] T.3 Laid-out-wait + predicate test (test-plan #F3, #X1): card 0-height on
...
- [ ] T.4 Degrade test (test-plan #X3, #X4, #X5): seek a hidden session
      ...
      scenarios.
- [ ] T.5 Ancestor-resolution + idempotence test (test-plan #E2, #E3, #E4):
...
- [ ] T.6 Backstop + Retry-toast + no-leak test (test-plan #E5, #X2): echo never
...
- [ ] T.7 Desktop-only gate test (test-plan #E1): render `SessionHeader` with
      `useMobile()` → true · assert no `session-header-seek-card` element.
      → verify: matches the mobile-layout scenario.
- [ ] T.8 Manual (test-plan: manual-only, #F5): open a buried session in ChatView,
...
- [ ] V.1 `npm run quality:changed` green (biome + tsc + tests).
- [ ] V.2 Manual: open a buried session in ChatView, click Seek, confirm the
