...
- [ ] 1.1 `trimBufferToLimit(buf, cap)` returns `{ dropped, toolEndDropped }` —
...
- [ ] 1.2 `evictIfNeeded()` returns the number of session buffers it deleted.
- [ ] 1.3 Add closure counters in `createMemoryEventStore`: `trimmedEventsTotal`,
...
- [ ] 1.4 Add `getTrimStats()` to the returned store handle →
  `{ trimmedEvents: { total, toolExecutionEnd, bySession }, evictedSessions }`.

## 2. Surface on `/api/health` — ADD
- [ ] 2.1 Add `eventStore` to the `registerSystemRoutes` deps (server.ts call
  site ~1132) and destructure it in `system-routes.ts`.
- [ ] 2.2 In the `/api/health` handler, add `storeTrim:
...
## 3. Tests
- [ ] 3.1 Store unit test: insert past `maxEventsPerSession + trimSlack` with a
...
- [ ] 3.2 Store unit test: exceed `maxCachedSessions` → `evictedSessions`
  increments; a session under both caps yields all-zero stats (no false counts).
- [ ] 3.3 Health-route test (extend `system-routes`/health test): `/api/health`
  returns a `storeTrim` object with the four fields; zero when nothing trimmed.

## 4. Validate
- [ ] 4.1 `npm test` green (new store + health tests, existing suites).
- [ ] 4.2 `openspec validate instrument-event-store-trim --strict`.
- [ ] 4.3 `npm run quality:changed` clean (tsc --noEmit; full suite; zero new
  Biome warnings on changed lines).

## 5. Decision follow-through (not code)
- [ ] 5.1 After a real-usage window, read `/api/health#storeTrim` on the live
