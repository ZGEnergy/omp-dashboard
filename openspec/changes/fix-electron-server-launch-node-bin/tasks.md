...
- [ ] 8.1 On a Mac with no dashboard running, double-click the Electron app icon, confirm loading page resolves to the connected UI within 15s.
- [ ] 8.2 Inspect `~/.pi/dashboard/server.log` and confirm post-banner output (`[server] listening on …`, etc.).
- [ ] 8.3 Verify the spawned child is the bundled Node: `ps -o pid,ppid,command -p <pid>` should show `<app>/Contents/Resources/node/bin/node --import …`.
- [ ] 8.4 Repeat after temporarily renaming the bundled `Resources/node/bin/node` to validate the system-Node fallback path (nvm Node).
- [ ] 8.5 Restore the bundled Node when done.
...
- [ ] 10.3 `npm run build` succeeds; `npm run electron:build` produces a Mac DMG that boots through the manual QA in §8. (Deferred — requires a full Electron build cycle, out of scope for automated implementation.)
