...
The dashboard server SHALL expose `POST /api/kb/reindex?cwd=<abs>` that indexes the folder's resolved sources via the shared `indexSource` primitive, without requiring a live pi session.

#### Scenario: Reindex a session-less worktree
- **WHEN** `POST /api/kb/reindex?cwd=W` is called for a worktree with no attached pi session
- **THEN** `indexSource` runs over `W`'s resolved sources in the dashboard-server process
- **AND** on completion `GET /api/kb/stats?cwd=W` reports `chunks > 0` and `indexed: true`
...
- **AND** the response references the in-flight job (`status: "running"`)

#### Scenario: Reindex is incremental
- **WHEN** `POST /api/kb/reindex?cwd=C` runs and no file under `C` changed since the last index
- **THEN** the mtime→sha256 gate skips unchanged files
- **AND** the response reports `changed: 0`
...
#### Scenario: Running job shows progress
- **WHEN** a reindex job for folder `C` is in progress (`indexing: true`)
- **THEN** the row shows an indexing state with an animated indicator
- **AND** the client polls stats until indexing completes
