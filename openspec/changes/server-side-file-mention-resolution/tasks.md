...
- [ ] 1.2 Unit tests for `resolveFileMention`: `~/.pi/...`→resolves under `~/.pi`;
...
- [ ] 2.1 Add `POST /api/file/resolve-mention` in `file-routes.ts`: body `{ cwd,
...
- [ ] 3.1 Extend the open/preview containment (`/api/file`, `/api/open-editor`,
...
- [ ] 4.1 Add ONE `~/` file-token branch to `linkify-tool-output.ts` (leading
...
- [ ] 4.2 `FileLink` resolves on click via `/api/file/resolve-mention` and opens
...
- [ ] 4.3 On resolve-request FAILURE (network/5xx/timeout, distinct from null),
...
- [ ] 5.1 `~/.pi` home file resolves: `~/.pi/dashboard/worktree-init-trust.json` exists, known cwd · resolve · `{resolved under <home>/.pi, kind tilde}` (test-plan #S2).
- [ ] 5.2 home outside `~/.pi` rejected: `~/.ssh/id_rsa`, known cwd · resolve · `null` (test-plan #S3).
- [ ] 5.3 tilde traversal escape: `~/../../etc/passwd`, known cwd · resolve · `null` after expand+containment (test-plan #S4).
- [ ] 5.4 relative resolves: `packages/server/src/routes/file-routes.ts` under cwd · resolve · resolved rooted at cwd, kind relative (test-plan #S5).
- [ ] 5.5 nonexistent → null and `~alice/x.ts` not expanded (test-plan #S6, #S7).
- [ ] 5.6 containment-before-stat: `fs.stat` spy asserts stat runs only after containment passes, never before (test-plan #S8).
...
- [ ] 5.7 untrusted cwd 403: `{cwd:"/etc",mention:"passwd"}`, `/etc` not a known session cwd · POST `/api/file/resolve-mention` · 403 and no `fs.stat` (test-plan #S1).
- [ ] 5.8 open route honors `~/.pi`: GET `/api/file` read of a resolved `~/.pi/agent/settings.json` with a project cwd · 200; a `~/.ssh/config` read · 403 (test-plan #S9, #S10).

L1 client tokenizer — extend `packages/client/src/lib/__tests__/linkify-tool-output.test.ts`:

- [ ] 5.9 tilde token: `~/.pi/dashboard/trusted-paths.json` · `tokenize()` · ONE file token, path retains `~/…`, no orphan `~`, join-coverage holds (test-plan #S11).
- [ ] 5.10 no regression: `Node.js` / `math.PI` / `and/or` · `tokenize()` · no new false-positive file token from the tilde branch (test-plan #S12).
...
- [ ] 5.11 click resolves + opens: link for `~/.pi/agent/settings.json`, resolve mocked → resolved · click · open called with server-resolved path (test-plan #S13).
- [ ] 5.12 null → inline not-found: resolve mocked → null · click · inline not-found affordance (strikethrough/disabled), NO open call (test-plan #S14).
- [ ] 5.13 request failure → fallback: resolve mocked → 5xx/reject · click · client-side `resolveLinkOrigin` open taken, rejection caught, not treated as null (test-plan #S15).
- [ ] 5.14 no double re-root: worktree session, resolve → absolute · click · open target equals server path exactly (test-plan #S16).
- [ ] 5.15 split-open resolves (G2): cwd-relative token in split-workspace · click · routes through resolve endpoint, no client short-circuit (test-plan #S17).
- [ ] 5.16 lazy render invariant: message with N mentions · mount · zero resolve calls until a click (test-plan #S18).
...
- [ ] 5.17 real message with `~/.pi/agent/settings.json` in the harness · click the link · preview/editor opens the resolved home file, not a `/`-rooted 404 (test-plan #S19).

## 6. Validate

- [ ] 6.1 `npx openspec validate server-side-file-mention-resolution --strict` passes.
- [ ] 6.2 `npm test` green; manual (test-plan #M1): remote/tunnel click→open
