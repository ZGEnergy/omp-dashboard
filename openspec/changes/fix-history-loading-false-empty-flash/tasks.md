...
- [ ] 1.1 Confirm the timeline on a large ended session: select card, observe
...
- [ ] 1.2 Confirm `App.tsx:651` 15s timer is the clearer via a breakpoint /
      log on `clearLoadingHistory` (node-inspect-debugger for the timer closure).

## 2. Client timer lifecycle (C)
- [ ] 2.1 Add `SUBSCRIBE_ACK_MS = 15000` and `HYDRATE_CEILING_MS = 90000`
      constants (co-locate with the existing timer-arming site in `App.tsx`).
- [ ] 2.2 Add `rearmLoadingHistory(setLoadingHistory, timersRef, id, ms)` to
...
- [ ] 2.3 In `useMessageHandler.ts` `event_replay` handler, when
...
- [ ] 2.4 Ensure the existing clear-on-first-content and clear-on-`isLast:true`
      edges (and `dataUnavailable`) still fire and tear down whichever timer is
      armed (short or long).

## 3. Server hydration heartbeat (B)
- [ ] 3.1 Add `HYDRATE_HEARTBEAT_MS = 10000` constant in
      `packages/server/src/browser-handlers/subscription-handler.ts`.
- [ ] 3.2 On the cold branch, immediately after the initial priming
...
- [ ] 3.3 Clear the interval in EVERY exit path of the `loadSessionEvents`
...
- [ ] 3.4 Confirm no heartbeat is started on the warm/in-memory branches (they
...
- [ ] 4.1 Cold path: priming marker keeps flag set past 15s; content batch clears.
- [ ] 4.2 Dead-link path: no priming marker → cleared at 15s.
- [ ] 4.3 Stuck-worker path: priming marker, no heartbeats, no content → cleared at 90s.
- [ ] 4.4 Heartbeat re-arm: priming + empty `{isLast:false}` beats spaced < 90s keep
      flag set past 90s; clears only after beats stop + ceiling elapses (or content).
- [ ] 4.5 Empty session: priming then `{events:[], isLast:true}` → cleared, placeholder.
- [ ] 4.6 Warm regression: content-only replay clears on first content < 15s.
### Server (packages/server/src/__tests__/subscription-handler.test.ts)
- [ ] 4.7 Cold subscribe emits ≥1 heartbeat marker before a delayed `loadSessionEvents` resolves.
- [ ] 4.8 Heartbeat stops after the first content batch; none after `isLast`.
- [ ] 4.9 Heartbeat stops on failure / `cancelled` / closed socket (no send to closed ws).

## 5. Spec + gates
- [ ] 5.1 `openspec validate fix-history-loading-false-empty-flash --strict`.
- [ ] 5.2 `npm run quality:changed` (biome + tsc + tests).
- [ ] 5.3 code-review gate on the diff.

## 6. Docs
- [ ] 6.1 Update `packages/client/src/lib/loading-history.ts` header + the
