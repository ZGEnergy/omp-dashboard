...
- **Session card flow launcher**: When a session has available flows (detected from the commands list), the session card shows a "Flows:" labeled action section with a "▶ Run Flow..." button that opens a searchable select dialog, then a task input dialog. Same searchable dialog pattern used for OpenSpec attach.
- **Searchable select dialog**: Shared `SearchableSelectDialog` component used by both flow launcher and OpenSpec attach. Centered overlay with search field, keyboard navigation (↑↓ Enter Esc), real-time filtering, label + description per item.
- **OpenSpec attach improved**: The OpenSpec attach picker now uses the same `SearchableSelectDialog` with rich descriptions showing lifecycle state, task progress, and artifact list. Section labeled "OpenSpec:" for visual distinction.
