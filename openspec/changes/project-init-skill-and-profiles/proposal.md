...
- `project-profiles`: A profile system. A profile is a directory bundling an `AGENTS.md` template, a `worktreeInit` hook, toolset toggles, and prompt files. Profiles resolve from shipped defaults (`coding`, `docs`) plus `~/.pi/project-profiles/`, with user profiles overriding shipped ones by name.
