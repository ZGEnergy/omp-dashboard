...
- [ ] T-E7 (test-plan #E7) all 5 wiring sites gated. input: non-mobile; badge on SessionList/board/mobileActions/SessionHeader/ComposerSessionActions · trigger: click P on each · observable: dialog opens on every surface, none navigates.
- [ ] T-F1 (test-plan #F1) dialog over view, URL unchanged, tabs. input: non-mobile, change with 4 artifacts, active session view · trigger: click P · observable: dialog mounted, underlying view still in DOM, `location.href` unchanged, tab bar P/D/S/T with P active.
- [ ] T-F2 (test-plan #F2) tab switch = local state, no history push. input: dialog open on P · trigger: click D tab · observable: design content, `history.length` + `location.href` unchanged.
- [ ] T-F3 (test-plan #F3) flex-wrapper full-height render (regression guard for flex-1 collapse). input: dialog open · trigger: render · observable: header + tab bar + content area visible, content boundingRect height > 0.
- [ ] T-F4 (test-plan #F4) close via Esc / backdrop / back. input: dialog open (3 edges) · trigger: Esc · backdrop click · reader back · observable: dialog unmounted each way, underlying view revealed unchanged.
- [ ] T-F5 (test-plan #F5) focus returns to triggering badge. input: dialog opened from badge B · trigger: close · observable: `document.activeElement` === badge B.
- [ ] T-F6 (test-plan #F6) resize into mobile auto-closes. input: dialog open at vw=1000 · trigger: resize to vw=700 · observable: dialog unmounted.
- [ ] T-F7 (test-plan #F7) ephemeral — no reload survival. input: dialog open · trigger: reload page · observable: after reload no dialog, base route URL.
- [ ] T-X1 (test-plan #X1) cold-load convergence. input: openspecMap has no entry for cwd · trigger: click badge, then WS replay populates map · observable: loading state → artifact content, no crash.
- [ ] T-X3 (test-plan #X3) change removed mid-dialog. input: dialog open on "ch" · trigger: WS drops "ch" from active map · observable: dialog flips to not-found, no exception.
- [ ] T-E8 (test-plan #E8) archive isolation (non-goal guard). input: archived-change badge in ArchiveBrowserView · trigger: click P · observable: archive reader renders (archive=true), no OpenSpecArtifactDialog, no navigate.
- [ ] T-E9 (test-plan #E9) mobile route unchanged (non-goal guard). input: useMobile()=true · trigger: click P · observable: full-page preview route, browser Back closes.

## Validate
- [ ] V1 `npm test` green.
- [ ] V2 `npx openspec validate openspec-artifact-dialog-desktop --strict` passes.
- [ ] V3 (test-plan #F9, manual-only) Manual: no badge surface renders inside an already-open Dialog (nested focus-trap latent-collision guard); archive browser artifact reading unaffected.
