...
- **MODIFIED files**:
  ...
  - `packages/shared/src/types.ts` — the `BashOutputData` interface gets optional `source?: "slash-exec"` field (NOT `protocol.ts`/`browser-protocol.ts` — `bash_output` payload is defined only in `types.ts`).
...
This change INSERTS one new routing step between the fix's step 9 (`extension command → pi.dispatchCommand`) and step 10 (`fall through to template expansion + sendUserMessage`): if the resolved template carries `executable: bash` frontmatter, run the body as bash and skip the LLM entirely. The fix HAS LANDED (archived 2026-05-09) and DID extract the shared `packages/extension/src/slash-dispatch.ts` helper (`tryDispatchExtensionCommand`). This change adds the exec-mode branch as a NEW check placed immediately after the `tryDispatchExtensionCommand` call in both call sites (`bridge.ts::sessionPrompt` and `command-handler.ts`'s slash else-arm) and BEFORE the `expandPromptTemplateFromDisk` fallback. Note: `slash-dispatch.ts` itself only matches JS extension commands (`pi.getCommands()` source); exec-mode `.md` templates are a separate disjoint check, not a modification of that helper's internals.
