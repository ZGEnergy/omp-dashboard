...
`ChatView` (`packages/client/src/components/ChatView.tsx`, 823 LOC) is the root of the dashboard's live agent-timeline UI: streaming text/thinking, tool-call bursts, inline terminals (xterm), file-preview + diff cards, skill-invocation cards, interactive UI requests, steering ghost-bubbles, a **TanStack-virtualized transcript** (`@tanstack/react-virtual` + `lib/chat-virtual-rows.ts`) with sticky-bottom auto-scroll driven through the virtualizer, FX-visibility gating (`useFxVisibility`), and per-session display prefs. A sibling package inside this monorepo wants to mount the **same live chat, at full fidelity, fed by the same pi dashboard WebSocket protocol**.

Investigation of the actual dependency graph:

- `ChatView` transitively reaches **107 local files** and **24 external packages** (the set now includes `@tanstack/react-virtual`, a declared `packages/client` dependency, since the transcript virtualized).
...
- **Bounded-height scroll parent** (new — drift): the transcript is now TanStack-virtualized, so the host must mount `ChatView` inside a container with a bounded (measurable) height. An unconstrained/auto-height parent starves the virtualizer of a scroll viewport and the transcript fails to size/scroll. `FilePreviewProvider`/`FilePreviewHost` are self-mounted inside `ChatView` — NOT a host-supplied provider.
...
- **No file relocation**: the 107-file subtree stays in `packages/client`. Full extraction into a standalone `packages/chat` library is out of scope (see Alternatives).
...
- **Extract a new `packages/chat` source-only library** (move all 107 files + event-reducer + tool-renderers): the clean long-term boundary, but a large mechanical import-rewrite across the whole client with real regression surface. Deferred; the subpath-export names introduced here are forward-compatible with it.
...
- **Carried runtime dependencies** (consumer must be able to resolve all, via workspace hoist): the 24 external packages the `ChatView` subtree reaches — notably `react`/`react-dom` (single copy), `wouter`, `@tanstack/react-virtual` (**new since proposal**), the xterm stack (`@xterm/xterm` + `addon-attach` + `addon-fit`), the git-diff-view stack (`@git-diff-view/{react,file,lowlight}` + `diff`), the markdown stack (`react-markdown`, `react-syntax-highlighter`, `remark-{gfm,math,frontmatter}`, `rehype-{katex,raw}`, `yaml`), `@mdi/{js,react}`, and the sibling workspace pkgs (`dashboard-plugin-runtime`, `pi-dashboard-client-utils`, `pi-dashboard-shared`, `pi-dashboard-subagents-plugin`). No `dependencies` are added to `packages/client` by this change; the barrel only re-exports what already ships.
- **Coupling made explicit**: host must mount `ThemeProvider` (throws if absent), `UiPrimitiveProvider` (originates in `@blackbelt-technology/dashboard-plugin-runtime`), `MobileProvider`, `SessionAssetsProvider`, `DisplayPrefsProvider`, api-context, and a wouter `Router`; provide a **bounded-height scroll container** (virtualized transcript); construct `ToolContext`; ensure single React, Vite JSX transform for the package, Tailwind content glob, and theme CSS vars. `FilePreviewProvider` is self-mounted by `ChatView`. Documented in `docs/embedding-chat-view.md`.
- **Code impact**: new hook (thin wrapper over `event-reducer`, incl. replay seq-reset), barrel re-exports + possibly a thin `ApiProvider` wrapper, minimal `exports` map (incl. `./package.json`), one topic doc. **This change itself declares no new dependency** — but the embed surface now transitively drags in `@tanstack/react-virtual` (added to `packages/client` by the transcript-virtualization work, commit `311de78f4`, *after* this proposal was written). The workspace consumer must resolve/hoist it along with the rest of the carried runtime set (below).
...
- **Out of scope**: relocating the 107-file subtree; npm-registry publishing of `chat-embed`; changing the WebSocket protocol; new UI.
