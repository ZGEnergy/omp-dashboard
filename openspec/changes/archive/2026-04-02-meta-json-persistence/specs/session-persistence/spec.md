...
**Reason**: Hidden sessions now persist in their own `.meta.json` with `hidden: true` instead of being excluded from a monolithic file.
**Migration**: Hidden state is stored per-session in `.meta.json`. No centralized exclusion needed.
