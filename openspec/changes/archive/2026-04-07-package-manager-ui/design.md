...
### 13. Dynamic import for pi-coding-agent

**Decision:** `pi-coding-agent` is an optional peer dependency not present in `node_modules`. The wrapper uses dynamic `import()` with `pathToFileURL()` resolving from `npm root -g`. Tries both `@mariozechner/pi-coding-agent` and `@oh-my-pi/pi-coding-agent`. Results are cached after first successful load.

**Why:** The dashboard can't bundle pi-coding-agent as a direct dependency — it's a globally installed CLI tool. Dynamic import with global resolution is the only reliable way to access its exports at runtime without requiring it in the project's `package.json` dependencies.

### 14. README rendering via MarkdownContent

**Decision:** Instead of a custom markdown-to-HTML parser, reuse the existing `MarkdownContent` component (react-markdown + remark-gfm + rehype-raw + syntax highlighting) for rendering package READMEs.

**Why:** npm READMEs contain both markdown and raw HTML. `MarkdownContent` already handles both via its rehype-raw plugin and includes syntax highlighting for code blocks. Reusing it avoids duplicating rendering logic and ensures visual consistency with the rest of the dashboard.

### 15. Merged Local/Global scope sections

**Decision:** Instead of three sections (Local, Global, Packages), the Resources Installed tab shows two merged sections: Local (loose files + local packages) and Global (loose files + global packages). Added a `scope` field to `PiPackageInfo` to distinguish package origin.

**Why:** Separating loose files from packages within the same scope created unnecessary visual fragmentation. Merging them by scope matches how pi itself organizes resources and reduces cognitive overhead — users think in terms of "local to this project" vs "available everywhere".

### 16. Collapsible resource hierarchy

**Decision:** All levels in the Resources Installed tab are collapsible with chevrons, defaulting to collapsed. Progressive indentation (16px per depth level) visually distinguishes nesting.

**Why:** The resource tree can be deep (scope → type → package → individual files). Defaulting to collapsed keeps the overview scannable, while expand-on-click lets users drill into specific areas.

### 17. Cross-scope installation badges

**Decision:** `PackageBrowser` fetches both own-scope and other-scope installed packages to show "Global", "Local", or "Global + Local" badges on package cards.

**Why:** Users need to know at a glance whether a package is already installed and where. Without cross-scope awareness, a user browsing local packages wouldn't know the package is already installed globally (and vice versa), leading to unnecessary duplicate installs.

### 18. Installed filter pill

**Decision:** Added an "installed" toggle filter that shows only installed packages, merging search results with synthetic entries for installed packages not present in search results.

**Why:** Users often want to manage what they already have. The filter surfaces all installed packages even if they don't appear in the current npm search (e.g., git-only packages or packages with different keywords).

### 19. WebSocket dispatch via DOM events

**Decision:** Package progress/complete messages are dispatched from `useMessageHandler` via `window.dispatchEvent(new CustomEvent("pi-package-event"))`, and component-level hooks listen via `addEventListener`.

**Why:** Package operations are transient and only relevant to components currently mounted (PackageBrowser, PackageCard). Using DOM events avoids coupling to shared React state architecture, prevents unnecessary re-renders of unrelated components, and keeps the package management hooks self-contained.
