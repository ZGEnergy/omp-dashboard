...
- **THEN** `manifest.json.npmCacheSha256` matches the SHA-256 of the tarball content
...
- **AND** verifies the extracted content SHA-256 against `manifest.npmCacheSha256`
...
- **WHEN** the extracted cache SHA-256 does not match `manifest.npmCacheSha256`
- **THEN** the installer aborts with an integrity error
- **AND** does NOT fall back to the registry
- **AND** preserves the extracted directory for debugging
