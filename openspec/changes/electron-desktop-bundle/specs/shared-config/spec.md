...
| `spawnStrategy` | `"tmux" \| "headless"` | `"headless"` | Strategy for spawning new pi sessions from the dashboard |
...
- **THEN** `loadConfig()` SHALL return all default values with `auth` as undefined and `electronMode` as `false`
