...
| `mobile-session-replay.spec.ts` | Mobile replay browser coverage. Session selection cold-replays the shared >256-event `long-transcript` fixture, asserts its visible tail is real text (not `…[truncated]`) and loading skeleton settle on a live socket with a clean terminal; Chromium CDP touch captures only the created session's WebSocket frames to prove the older `subscribe` carries finite `fromSeq > 0` and its matching `event_replay` terminal is authoritative and error-free, alongside older-page anchor identity/offset; hidden detail stays inert; WebKit asserts foreground reconnect sends cold `subscribe lastSeq:0`, then transcript/image rehydrate. |
| `model-preference-sync.spec.ts` | Focused rendered-browser two-way model/thinking sync. Dashboard selections round-trip through spawned Pi bridge; fixture-only `/e2e_model_sync` drives ordered Pi-side model/thinking changes including clear→off. Requires `PI_E2E_SEED=1` and headless spawn strategy. |
