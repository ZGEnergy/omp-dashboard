...
| `mobile-session-replay.spec.ts` | Mobile replay browser coverage. Session selection cold-replays the shared >256-event `long-transcript` fixture, asserts its visible tail is real text (not `…[truncated]`) and loading skeleton settle on a live socket with a clean terminal; Chromium CDP touch checks older-page anchor identity and offset; hidden detail stays inert; WebKit asserts foreground reconnect sends cold `subscribe lastSeq:0`, then transcript/image rehydrate. |
