...
| `mobile-session-replay.spec.ts` | Mobile replay browser coverage. Chromium CDP touch checks older-page anchor identity and offset; hidden detail stays inert; WebKit asserts foreground reconnect sends cold `subscribe lastSeq:0`, then transcript/image rehydrate. |
