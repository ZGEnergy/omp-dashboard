...
function toRelativeImportPath(fromDir: string, toFile: string): string {
  let rel = path.relative(fromDir, toFile).replace(/\.(tsx?|jsx?)$/, "");
  // Use POSIX separators so the import string is portable across OSes.
...
function generateRegistryContent(entries: PluginEntry[], repoRoot: string): string {
  ...
  for (const entry of entries) {
    // Relative import from the generated file's directory to the plugin entry.
    // Keeps the generated file machine-independent (no absolute /Users/... leaking into git).
    ...
    const importPath = toRelativeImportPath(generatedDir, entry.clientEntryPath!);
    const namedRefs = [
      ...new Set(
