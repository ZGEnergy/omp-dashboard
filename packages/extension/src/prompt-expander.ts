...
function findPromptTemplates(cwd: string): Map<string, string> {
  ...
  // .omp is the primary project directory; .pi is a legacy fallback for older projects.
  // .omp is scanned AFTER .pi so .omp entries overwrite .pi (last-wins via Map.set).
  const dirs = [
    join(cwd, ".pi", "prompts"),
    join(cwd, ".pi", "skills"),
