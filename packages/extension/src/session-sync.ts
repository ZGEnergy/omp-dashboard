 ...
 * Routes every synthesized event through the shared `replayEntriesAsEvents`
 * replay-options seam so legacy inline tool-result images become bounded
 * `pi-asset:` references instead of inline bodies. Inline image bytes are
 * hashed through the existing asset machinery (`hashBytes`: sha256 → 16 hex
 * chars) and each deduped `asset_register` is emitted BEFORE the referencing
 * `event_forward`, matching the live `maybeInlineAssistantImages` ordering
 * contract. The per-replay dedup set guarantees one `asset_register` per unique
 * hash even when the same image appears across multiple tool results. Malformed
 * content is recovered (the seam is non-throwing) and never aborts the whole
 * session.
 */
