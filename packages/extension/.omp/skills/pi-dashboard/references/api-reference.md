...
Base URL: `http://localhost:{port}` (default port: `8000`, configurable in `~/.omp/dashboard/config.json`).
...
Shut down an omp session (terminates the process, not the server).
...
Spawn a new omp session in a directory.
...
Read an omp resource file (localhost-only, restricted to allowed locations).
