...
description: >
  ...
  AGENTS.md, .omp/settings.json (with a worktreeInit hook + toolset), and prompt
...
3. Substitute the chosen stack's values for the `{{…}}` placeholders when
   ...
   remains in `AGENTS.md` or `.omp/settings.json` — an unfilled placeholder means
...
- `./.omp/settings.json`    ← `<skill>/profiles/<p>/settings.json.tmpl`
- `./.omp/prompts/*.md`     ← each file in `<skill>/profiles/<p>/prompts/`

When the chosen profile has `dox: true`, ALSO name in the preview:

- the DOX doctrine seed appended to `./AGENTS.md` (see Step 5)
- the kb toolset flip written to `./.omp/dashboard/knowledge_base.json`

When the chosen profile is `coding`, ALSO disclose the possible side effect (see
Step 4b) — NOT a file write:

- may offer to `omp install` the `eng-disciplines` skills **user-globally**
  (writes `~/.omp/agent/settings.json`). This is machine-wide: the skills become
  ...
  (the install runs `omp install npm:…`); the scaffolded repo itself gains no
  dependency.

**Idempotency:** before writing, check whether `./AGENTS.md` or
`./.omp/settings.json` already exist. If they do, ask before overwriting — never
clobber silently.
...
**Validate the hook.** After writing `./.omp/settings.json`, confirm its
...
```bash
omp list 2>/dev/null | grep -q pi-dashboard-eng-disciplines \
  || stat ~/.omp/agent/npm/node_modules/@blackbelt-technology/pi-dashboard-eng-disciplines >/dev/null 2>&1
```
...
> machine. Writes `~/.omp/agent/settings.json` (user-global, not project-local).

- **Yes** → run `omp install npm:@blackbelt-technology/pi-dashboard-eng-disciplines`
...
- **No / install failed** → append ONE line under the `## Discipline Skills`
  ...
  > Discipline skills not detected — run `omp install npm:@blackbelt-technology/pi-dashboard-eng-disciplines` to activate the checkpoints above.
...
Then write `./.omp/dashboard/knowledge_base.json` (when absent) enabling the
