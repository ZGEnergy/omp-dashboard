...
> The `agent-browser` CLI is not installed. Install it as an omp extension so
> the `browser` tool is registered in your omp session too:
>
> ```
> omp install npm:pi-agent-browser
> ```
>
> Then re-invoke the skill.

Do **not** attempt `npm install`, `omp install`, or any other install command
...
- **User-local override**: if the user's project has its own
  `.omp/skills/browser/` skill, pi's local-wins precedence applies and
