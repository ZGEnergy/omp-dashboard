...
| `ForkPendingContext.tsx` | Fork-pending signal shared by every fork control. Exports `useForkPendingController(onSettle?)` → `{pendingKeys, beginFork(key,sessionId,requestId):boolean, settleFork(requestId)}`, `ForkPendingProvider`, `useForkPending()` → `isForkPending(key)`, `FORK_PENDING_TIMEOUT_MS` (30 s). Key = `entryId` for fork-from-message, else `sessionId`. `beginFork` returns false on a duplicate = the client dedup. `onSettle(sessionId)` fires on every settle path so the source session's optimistic `resuming` clears. NOT routed through `spawnResult`/`SessionList` (unmounted in mobile chat view). See change: fork-action-opens-an-empty-chat. |
...
| `push-notification-payload.ts` | Pure link-only mapping for Web Push notification payload/notification args, shared with the dependency-free service worker contract. Keep private session content out of payloads. See change: add-server-push-notifications. |
...
| `replay-retention.ts` | Shared 6 MiB hot-transcript and durable-cache cap. Exports `DEFAULT_REPLAY_RETENTION_BYTES`. |
...
| `session-replay-ledger.ts` | Canonical contiguous replay ledger. Validates source generation and sequence continuity; retains only newest 10 MiB tail; emits head-eviction signal for atomic state rebuild; older pages remain available. `older` admissions SKIP `trimRetained` — retention evicts the oldest seq and load-older adds there, so trimming discarded the page just fetched and back-paging silently stalled (measured: stuck at seq 15,060 of 19,677). Cap re-applies on the next live admit and on `setMaxRetainedBytes`. See change: cheap-older-pages. |
