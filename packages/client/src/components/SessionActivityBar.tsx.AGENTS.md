...
Reads `useMobile()` (no longer hook-free). Mobile: stop renders ≥44px with a resting `text-yellow-400` accent (hover-only accent never fires on touch) and icon `size=0.7`; `e.stopPropagation()` keeps a hit from triggering `SessionCard`'s `<li onClick>` navigation. Desktop unchanged. See change: mobile-stop-buttons-not-visible.
