    ...
    <div className="flex flex-col h-full min-h-0 bg-[var(--bg-primary)]" data-testid="openspec-board">
      ...
      <div className="flex items-center gap-3 px-4 py-2.5 bg-[var(--bg-secondary)] border-b border-[var(--border-primary)] sticky top-0 z-10 flex-wrap">
        <button onClick={onBack} className="text-blue-400 hover:text-blue-300 text-[13px] flex items-center gap-1" data-testid="board-back">
          <Icon path={mdiArrowLeft} size={0.6} /> {i18nT("common.back2", undefined, "Back")}
        </button>
        <span className="text-[var(--text-primary)] font-semibold text-[13px]">
          {i18nT("openspec.openspec", undefined, "OpenSpec")} <span className="text-[var(--text-tertiary)] font-normal board-crumb-dim">· {folderName} · {data.changes.length} changes</span>
        </span>
        ...
        <button onClick={onOpenSpecs} className="text-[11px] px-2.5 py-1 rounded-md border text-cyan-400 border-cyan-500/40 bg-cyan-500/5 hover:text-cyan-300" data-testid="board-specs">
          <Icon path={mdiFileDocumentOutline} size={0.5} className="inline mr-0.5" />{i18nT("openspec.specs", undefined, "Specs")}
        </button>
        <button onClick={onOpenArchive} className="text-[11px] px-2.5 py-1 rounded-md border text-purple-400 border-purple-500/40 bg-purple-500/5 hover:text-purple-300" data-testid="board-archive">
          <Icon path={mdiArchiveOutline} size={0.5} className="inline mr-0.5" />{i18nT("openspec.archive", undefined, "Archive")}
        </button>
        <button onClick={() => setProposalDialogGroup({ open: true, groupId: UNGROUPED })} className="text-[11px] px-2.5 py-1 rounded-md border text-blue-400 border-blue-500/40 bg-blue-500/5 hover:text-blue-300" data-testid="board-new-proposal">
          <Icon path={mdiPlus} size={0.5} className="inline mr-0.5" />{i18nT("openspec.newProposal", undefined, "New proposal")}
        </button>
      </div>
      ...
      <div className="flex items-center gap-2.5 px-4 py-2 bg-[var(--bg-secondary)] border-b border-[var(--border-primary)] flex-wrap" data-testid="board-filterbar">
        <input
          ...
          placeholder={i18nT("openspec.filterProposalsSessions", undefined, "Filter proposals & sessions…")}
        ...
        <span className="text-[9px] text-[var(--text-muted)] uppercase tracking-wider">{i18nT("common.state", undefined, "State")}</span>
        ...
        <span className="w-px h-[18px] bg-[var(--border-secondary)]" />
        <span className="text-[9px] text-[var(--text-muted)] uppercase tracking-wider">{i18nT("session.session", undefined, "Session")}</span>
        <div className="flex gap-1">
          {SESS_PILLS.map((p) => (
      ...
      <DndContext sensors={sensors} collisionDetection={closestCenter} onDragStart={handleDragStart} onDragEnd={handleDragEnd}>
        <div ref={boardScrollRef} className="flex gap-3 p-4 items-start overflow-x-auto flex-1 min-h-0 board-columns" data-testid="board-columns">
          ...
          >
            {i18nT("common.addGroup", undefined, "+ Add group")}
          </button>
        </div>
        <DragOverlay>
          {activeDrag ? (
      ...
      <div className="px-4 pb-3 text-[var(--text-muted)] text-[11px]">
        {i18nT("common.dragColumnHeadersToReorderGroups", undefined, "Drag column headers to reorder groups. Drag proposal cards between columns to reassign group. Click a session row to open its chat view.")}
      </div>
      ...
      {addGroupOpen && (
        <DialogPortal>
          <ModalShell onClose={() => setAddGroupOpen(false)} title={i18nT("common.addGroup2", undefined, "Add group")}>
...
function GroupCreateForm({ onSave, onCancel }: { onSave: (name: string, color: string) => void; onCancel: () => void }) {
  ...
  return (
    <div className="space-y-2">
      <input autoFocus value={name} onChange={(e) => setName(e.target.value)} placeholder={i18nT("common.groupName", undefined, "Group name")}
      ...
      <div className="flex gap-1.5 justify-end">
        <button onClick={onCancel} className="text-[10px] px-2 py-0.5 rounded border border-[var(--border-secondary)] text-[var(--text-muted)]">{i18nT("common.cancel", undefined, "Cancel")}</button>
        <button disabled={!name.trim()} onClick={() => name.trim() && onSave(name.trim(), color)} className="text-[10px] px-2 py-0.5 rounded border border-blue-500/50 text-blue-400 hover:bg-blue-500/10 disabled:opacity-40" data-testid="add-group-save">{i18nT("common.create", undefined, "Create")}</button>
      </div>
    </div>
  );
}
      ...
      <div className="bg-[var(--bg-primary)] border border-[var(--border-primary)] rounded-xl p-4 w-[330px]" onClick={(e) => e.stopPropagation()} data-testid="new-proposal-dialog">
        ...
        {gitWorktreeEnabled && (
          <div className="flex items-center gap-2 mb-2.5">
            ...
            <label htmlFor="np-wt" className="text-[12px] text-[var(--text-secondary)]">{i18nT("worktree.createInANewWorktreeOs", undefined, "Create in a new worktree (os/<name>)")}</label>
          </div>
        )}
