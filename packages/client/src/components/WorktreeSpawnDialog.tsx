...
interface Props {
  ...
  cwd: string;
  /** Optional initial value for the new-branch input. */
  initialBranch?: string;
  /** Optional change name forwarded through `onSpawn.opts.attachProposal`. */
  ...
  onCancel: () => void;
  /** Optional hook for rendering a pending spawn placeholder. */
  onSpawnStart?: (parentCwd: string) => void;
  /** Optional hook removing the pending spawn placeholder after failure. */
...
export function WorktreeSpawnDialog({ cwd, onSpawn, onCancel, initialBranch, attachProposal, onSpawnStart, onSpawnAbort }: Props) { const [data, setData] = useState<LoadedData | null>(null);
...
const buildOpts = useCallback(
  (gitWorktreeBase?: string): { gitWorktreeBase?: string; attachProposal?: string } => ({
