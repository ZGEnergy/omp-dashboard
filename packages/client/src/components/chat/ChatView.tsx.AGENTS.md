...
Scroll: one owner. Mobile tail effect follows latest until real touch/wheel says history. Only `NAVIGATING_BOTTOM` chases virtual row measurement. Exhausted older history hides affordance as soon as `hasMoreOlder` false. `overflowAnchor: "none"`. Browser anchoring bad here. Image decode queues guarded `resizeItem` row measurement. One guarded rAF writes mobile scroll. Desktop writes synchronously and keeps saved scroll state. See change: stabilize-mobile-follow-and-exhausted-history. Partial replay windows show a non-virtual notice; completed older pages show tool-only or ordinary feedback and may auto-discover up to three pages while preserving the captured anchor.
...
See change: persistent-pending-interactive-prompt-bar — derives the latest pending non-widget `interactiveUi` from `filteredMessages`, removes only that active row from virtualized `displayRows`, and renders one `InteractiveUiCard` prompt bar after (outside) `chat-scroll-container`. Resolved/cancelled prompts remain inline historical rows; widget-bar prompts stay out of both surfaces.
