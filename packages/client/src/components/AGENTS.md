...
| `ToolBurstGroup.tsx` | Renders temporal BURST group (data from `lib/group-tool-bursts.ts`). Exports `ToolBurstGroup({burst,toolContext})`. Auto-expand while any member runs / auto-collapse on done via derived `expanded = override ?? isRunning` (nullable `override` cell, no effect; finding 4). Header honest: running `⟳ Working · N done · $ <live cmd>`, done `✓ N tool calls · breakdown · duration` — NO fabricated total (finding 1); counts over UNDERLYING calls (nested ×N contributes N; finding 5). Body = `max-h-[190px]` scrollbox of `ToolCallStep` + nested `CollapsedToolGroup` + absorbed narration (`thinking`/non-empty `assistant`→inline text, `data-testid=tool-burst-narration`; empty/separator skipped). Consumes semantic-first `burst.items`. Prefs-gated via `toolCallPrefKey`; null if zero visible. Live-command chip via `getSummary`. See changes: group-tool-call-bursts, collapse-tool-calls-across-narration. |
