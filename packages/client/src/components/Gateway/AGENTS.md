...
| `GatewayPairQR.tsx` | Connect-a-device — two QR kinds by transport (D1). Pairing QR = TLS-only `{v,id,code,urls[]}` (re-guarded by `guardPairingUrls`, task 8.3) + base64url `pi:pair:v1.…` copy-string (task 8.4). Link QR = bare URL per no-TLS endpoint (task 8.2, no payload/crypto/bearer). Endpoint mini-list marks `in QR`/`excluded`. Typed compare-code approval (D12). Exports `GatewayPairQR`. |
