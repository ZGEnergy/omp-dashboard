...
describe("SessionReplayController", () => {
  ...
  it("includes admitted/current skipped ranges in accepted and persist effects for cold, delta, and older network frames", () => {
    const effects = {
      send: vi.fn(),
      apply: vi.fn(),
      replace: vi.fn(),
    ...
    // 1. Cold network frame carrying skippedSeqRanges
    const cold = controller.begin("s-net", "cold", "source-a");
    const coldFrameMsg = {
      ...frame(cold.requestId!, [entry(1), entry(101)], true, "source-a"),
      sessionId: "s-net",
      skippedSeqRanges: [{ fromSeq: 2, toSeq: 100 }],
      windowMinSeq: 1,
      windowMaxSeq: 101,
    };
    controller.handle(coldFrameMsg);

    expect(effects.apply).toHaveBeenCalledWith("s-net", [entry(1), entry(101)]);
    expect(controller.ledger("s-net").skippedSeqRanges).toEqual([{ fromSeq: 2, toSeq: 100 }]);
    expect(effects.persist).toHaveBeenLastCalledWith(
      "s-net",
      [entry(1), entry(101)],
      [{ fromSeq: 2, toSeq: 100 }],
      "append",
    ...
    // 2. Delta network frame carrying additional skippedSeqRanges
    const delta = controller.begin("s-net", "delta", "source-a");
    const deltaFrameMsg = {
      ...frame(delta.requestId!, [entry(105)], true, "source-a"),
      sessionId: "s-net",
      replayKind: "delta" as const,
      skippedSeqRanges: [{ fromSeq: 102, toSeq: 104 }],
      windowMinSeq: 105,
      windowMaxSeq: 105,
    };
    controller.handle(deltaFrameMsg);

    expect(effects.apply).toHaveBeenLastCalledWith("s-net", [entry(105)]);
    expect(controller.ledger("s-net").skippedSeqRanges).toEqual([
      { fromSeq: 2, toSeq: 100 },
      { fromSeq: 102, toSeq: 104 },
    ]);
    expect(effects.persist).toHaveBeenLastCalledWith(
      "s-net",
      [entry(105)],
    ...
    // 3. Older network frame carrying skippedSeqRanges
    const older = controller.begin("s-net", "older", "source-a", "anchor-net");
    const olderFrameMsg = {
      ...frame(older.requestId!, [entry(0)], true, "source-a"),
      sessionId: "s-net",
      replayKind: "older" as const,
      skippedSeqRanges: [{ fromSeq: -10, toSeq: -1 }],
      windowMinSeq: -10,
      windowMaxSeq: 0,
    };
    controller.handle(olderFrameMsg);

    expect(effects.replace).toHaveBeenCalledWith(
      "s-net",
      [entry(0), entry(1), entry(101), entry(105)],
      { requestId: older.requestId, anchorToken: "anchor-net" },
    );
    expect(controller.ledger("s-net").skippedSeqRanges).toEqual([
      { fromSeq: -10, toSeq: -1 },
      { fromSeq: 2, toSeq: 100 },
