...
| `zrok.ts` | zrok provider (child, public-only). Exports `zrokChildSpec` (`ChildProviderSpec`), `zrokRuntime` (`ChildTunnelRuntime` singleton), `ZrokProvider` (`TunnelProvider`), plus zrok-specific `detectZrokBinary`/`loadZrokEnv`/`releaseShare`/`_resetBinaryCache`/`_setBinaryAvailable` re-exported by `../tunnel.ts`. Holds reserve/release, `~/.zrok2` enrollment. `urlRegex` + `normalizeUrl` accept zrok v1 (`https://<t>.share.zrok.io`) AND v2 (bare `<t>.shares.zrok.io`, scheme prepended). See change: add-tunnel-providers, zrok-v2-quick-connect. |
