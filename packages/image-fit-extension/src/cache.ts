 ...
 *
 * Layout:
 *   <os.tmpdir()>/pi-image-fit/<sessionScope>/<hash>.webp
 *
...
export function scopeFor(sessionScope: string): CacheScope {
  ...
  return {
    dir,
    filePath: (hash) => path.join(dir, `${hash}.webp`),
  };
}
...
/**
 * Returns true if a cache file exists for `hash`.
 * Any fs error is treated as "no cache hit" — the caller will rebuild.
 */
export async function hasCached(scope: CacheScope, hash: string): Promise<boolean> {
  try {
    const st = await fs.stat(scope.filePath(hash));
    return st.isFile();
  } catch {
    return false;
  }
