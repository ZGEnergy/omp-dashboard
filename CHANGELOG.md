...
### Fixed
- **Electron wizard no longer logs `Failed to run npm root -g: spawnSync npm ENOENT` at first launch.** Root cause: pi's `DefaultPackageManager` was instantiated by the dashboard server before the `ToolRegistry` could resolve `npm`. On a clean macOS Electron install, `~/.pi-dashboard/node/` does not exist yet, and GUI processes don't inherit the user shell's PATH (no `/opt/homebrew/bin`), so the registry returned a bare `"npm"` that ENOENT'd on spawn. Fix: new `electronBundledRuntimeStrategy` probes `<process.resourcesPath>/node/...` (the bundled Node distribution shipped inside the Electron app), inserted into both `npm` and `node` strategy chains after `managed-runtime` and before `where`. `nodeScriptToArgv` now prepends the node interpreter on all platforms for `.js` paths (not just Windows), so the resolved `npm-cli.js` spawns as `[<bundled-node>, <npm-cli.js>]` without depending on PATH-resolvable `node`. (change: `fix-electron-wizard-npm-root-enoent`)
