...
### Added
- **Recommended extensions can declare + live-probe external requirements.** `RecommendedExtension` gains an optional `requires` (`piExtensions` / `binaries` / `services`), reusing the dashboard-plugin schema and probe (`runRequirementProbesFor`); `GET /api/packages/recommended` returns a structured `requirements` report + `missingRequirements`, and the Recommended Extensions card renders per-requirement satisfied (✓) / missing (⚠) badges. Populated for `pi-agent-browser` (`agent-browser` binary). Deliberately NOT declared for native npm deps (hermes→better-sqlite3) or unknown services to avoid always-red requirements. (change: `align-pi-080-and-publish-baseline-packages`)
- **Five extensions promoted to the recommended manifest.** `RECOMMENDED_EXTENSIONS` (`packages/shared/src/recommended-extensions.ts`) grows from 7 to 12 entries: `context-mode` (`strongly-suggested` — sandboxed code execution over large outputs + FTS5 knowledge base, `ctx_*` tools), plus `pi-hermes-memory`, `@ricoyudog/pi-goal-hermes`, `@blackbelt-technology/pi-model-proxy`, and `pi-simplify` (all `optional`). All are npm-sourced. The Recommended Extensions card renders the new rows via the existing server-side enrichment (no client change). (change: `align-pi-080-and-publish-baseline-packages`)
- **Baseline npm publishes at 0.5.4.** Four previously-unpublished public workspaces now exist on the `@blackbelt-technology` scope at `0.5.4` so the next coordinated `release-cut` bumps them in lockstep: `pi-image-fit-extension` (renamed from `pi-image-fit`), `pi-dashboard-kb`, `pi-dashboard-kb-extension`, `frontend-mockup-loop`. `packages/kb` gains a real tsc build (`tsconfig.json` → `dist/`, `build` + `prepublishOnly` scripts) so its `bin.kb` ships a runnable `dist/cli.js`; `publishConfig.access:"public"` added (was missing, the reason the first scoped publish never landed); `files` excludes `src/__tests__`. (change: `align-pi-080-and-publish-baseline-packages`)
...
### Changed
- **pi dependency bumped to `^0.80.2`** (was `^0.78.0`) in `packages/server/package.json` + bundled copy + lockfile. Picks up the pi 0.79.x/0.80.x line; the 0.80.0 pi-ai root→`/compat` entrypoint move is runtime-aliased by pi's extension loader and the dashboard's only pi-ai use is a dynamic `any`-typed import, so no source migration was required. Does NOT touch `piCompatibility` (the floor series owns that). (change: `align-pi-080-and-publish-baseline-packages`)
- **pi-hermes-memory is now the default memory backend.** `pi-flows` manifest `source` switched from the license-blocked git URL to `npm:@blackbelt-technology/pi-flows` (still absent from `BUNDLED_EXTENSION_IDS` until upstream declares a license; git-installed pi-flows still satisfies the entry via `sourcesMatch` git↔npm). **Migration**: image-fit's recommended id is now `@blackbelt-technology/pi-image-fit-extension`; existing installs of the old `@blackbelt-technology/pi-image-fit` are not auto-migrated by `sourcesMatch` (npm renames are not spanned) — swap the `packages[]` entry in `~/.pi/agent/settings.json` or reinstall via the Recommended Extensions card. (change: `align-pi-080-and-publish-baseline-packages`)
...
- **Process-level crash safety net.** `cli.ts` now installs top-level `unhandledRejection` + `uncaughtException` handlers (logged with a stable `[crash-safety]` prefix, never `process.exit()`) so a misbehaving plugin cannot kill the dashboard under Node 25's fatal-by-default async-fault policy. The daemon harness still covers real signal/exit crashes.
