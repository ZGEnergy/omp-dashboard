...
- **Triggers** (evaluated by the pure helper `isUnreadTrigger(eventType, before, after, payload)` in `event-status-extraction.ts`):
  ...
  2. Session's `currentTool` becomes an input-needed tool (`"ask_user"` or core `"ask"`) — input is requested.
...
- **Render precedence** (`SessionCard.tsx::getCardPulseClass`): input-needed (`ask_user` / core `ask`, purple) > `streaming || resuming` (yellow) > `unread` (cyan) > none. Streaming with `unread: true` shows yellow stripes; when streaming ends with the session still unviewed, the trigger fires, the card flips to cyan. The `card-unread-pulse` CSS class reuses the `card-working-stripes-scroll` and `card-working-opacity-pulse` keyframes verbatim — only the stripe and tint colors change to cool cyan (`rgba(34, 211, 238, 0.18)` and `rgba(34, 211, 238, 0.07)`). Cyan was selected to occupy its own corner of the dashboard palette (distant from yellow, purple, green, red). Reduced-motion users see a static cyan-tinted background, matching the working-pulse arm.

### Web Push notifications (Phase 1; PWA only)

Web Push is the only shippable notification transport in Phase 1. It is opt-in: `push.enabled` defaults to `false`; when disabled, the server does not construct a dispatcher, mount `/api/push/*`, or generate VAPID keys. `coalesceWindowMs` defaults to 30 seconds (config validation clamps it to 5–300 seconds). The transport type retains a typed `"web-push" | "fcm"` seam, but only Web Push delivery is implemented.

**Lifecycle:**

1. When `push.enabled` is true, `server.ts` creates the token registry, loads or generates the VAPID keypair, and wires the optional dispatcher before event wiring. Registered routes are network/auth-gated: `GET /api/push/vapid-public-key`, `POST /api/push/register`, `DELETE /api/push/register/:tokenId`, and `POST /api/push/test`. A Web Push transport is created only when `push.webPush.contactEmail` is present.
2. The PWA bootstrap registers `/sw.js`. `usePushSubscription` feature-detects Service Worker + Push support, fetches the VAPID public key, recovers an existing browser subscription, and idempotently re-registers it after refresh. Explicit subscribe invokes the browser permission API, creates a `PushSubscription`, and registers it; unsubscribe removes the browser subscription and server token. The existing Settings section exposes enable/disable/test and an iOS installed-PWA hint.
3. `event-wiring.ts` evaluates the pure `isUnreadTrigger(eventType, before, after, payload)` classifier once. For live events only, and only when `viewedSessionTracker.isViewedByAnyone(sessionId) === false`, the existing any-viewer/non-replay gate updates unread state. After that gate, the applicable trigger is classified into `actions-required` or `claude-decides`; current saved push preferences can suppress only the matching `pushDispatcher?.fanout(sessionId, event)` attempt, never unread stamping or broadcast. Replayed events and events viewed by any browser do not push. The fanout call is fire-and-forget and is never awaited, so transport latency cannot delay WebSocket event forwarding.
4. `PushDispatcher` builds a compact session-link payload, coalesces attempts per `(sessionId, deviceToken)` during the configured window, and sends through the Web Push adapter. Successful tokens are touched; Web Push `410 Gone` subscriptions are removed; individual transport failures are isolated. The service worker displays the notification and `notificationclick` opens the payload URL back to the session.

**Persistence and scope:** Push subscriptions are stored at `~/.pi/dashboard/push-tokens.json`; VAPID keys are stored at `~/.pi/dashboard/push-vapid.json`. Both use atomic JSON writes with owner-only `0600` permissions. Private VAPID material and complete subscription endpoints are not logged. FCM delivery, Capacitor/native delivery, new permission approval/deny UX, and full elicitation UX remain deferred; the existing browser permission API and PromptBus renderers are not a promise of those later surfaces.
...
**Core `ask` registration:** The bridge calls `registerAskTool(pi)` at factory load, before pi snapshots the extension tool registry. This re-exposes the core-named `ask` tool for dashboard headless sessions whose create-time `hasUI` gate would otherwise omit it. Its execution uses the same patched `ctx.ui.*` methods and PromptBus path as `ask_user`; the runtime `ask_user` registration remains at `session_start` to avoid static-name conflicts.
