...
export function EditorView({ cwd, onClose }: Props) {
  ...
  const startEditor = useCallback(async () => {
    ...
    try {
      const res = await fetch("/api/editor/start", {
        method: "POST",
  ...
  useEffect(() => {
    ...
    const sendHeartbeat = async () => {
      try {
        const res = await fetch(`/api/editor/${editorInfo.id}/heartbeat`, { method: "POST" });
        const data = await res.json();
        if (!data.success) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/editor/${editorInfo.id}/theme`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  const stopEditor = useCallback(async () => {
    ...
    await fetch(`/api/editor/${editorInfo.id}/stop`, { method: "POST" }).catch(() => {});
    setEditorInfo(null);
    setState("loading");
    onClose?.();
  }, [editorInfo, onClose]);
