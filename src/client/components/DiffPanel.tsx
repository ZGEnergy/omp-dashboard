...
import type { FileChangeEvent, FileDiffEntry } from "../../shared/diff-types.js";
...
export function DiffPanel({ file, selection, sessionId }: DiffPanelProps) {
  ...
  useEffect(() => {
    ...
    setFileContent(null);
    setFileError(null);
    fetch(`/api/session-file?sessionId=${encodeURIComponent(sessionId)}&path=${encodeURIComponent(file.path)}`)
      .then((res) => res.json())
      .then((body) => {
        if (body.success) {
