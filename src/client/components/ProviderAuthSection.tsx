...
async function fetchStatus(): Promise<ProviderAuthStatus[]> {
  const res = await fetch("/api/provider-auth/status");
  return res.json();
}
...
function OAuthProviderRow({ provider, onChanged }: { provider: ProviderAuthStatus; onChanged: () => void }) {
  ...
  const startAuthCode = async () => {
    ...
    setError(null);
    try {
      const res = await fetch("/api/provider-auth/authorize", {
        method: "POST",
      ...
      const handleCallback = async (cbData: any) => {
        ...
        if (cbData.code) {
          try {
            const exRes = await fetch("/api/provider-auth/exchange", {
              method: "POST",
              headers: { "Content-Type": "application/json" },
  ...
  const startDeviceCode = async (domain?: string) => {
    ...
    try {
      const res = await fetch("/api/provider-auth/device-code", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      pollingRef.current = setInterval(async () => {
        try {
          const statusRes = await fetch(`/api/provider-auth/device-status/${data.flowId}`);
  ...
  const handleSignOut = async () => {
    ...
    setError(null);
    try {
      await fetch(`/api/provider-auth/${provider.id}`, { method: "DELETE" });
      onChanged();
    } catch (err: any) {
      setError(err.message);
    }
...
function ApiKeyRow({ provider, onChanged }: { provider: ProviderAuthStatus; onChanged: () => void }) {
  ...
  const handleSave = async () => {
    ...
    setError(null);
    try {
      const res = await fetch("/api/provider-auth/api-key", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
