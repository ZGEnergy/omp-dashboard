...
async function fetchFile(cwd: string, filePath: string): Promise<string> {
  const res = await fetch(`/api/file?cwd=${encodeURIComponent(cwd)}&path=${encodeURIComponent(filePath)}`);
...
async function fetchDir(cwd: string, dirPath: string): Promise<string[]> {
  const res = await fetch(`/api/file?cwd=${encodeURIComponent(cwd)}&path=${encodeURIComponent(dirPath)}`);
  const body = await res.json();
